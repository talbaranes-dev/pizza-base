...
| 5.6 | **orchestrator (self)** | Seed initial RTDB state at `settings/`: `storeOpen = true` (so customer site shows OPEN immediately — otherwise `null` reads as closed) **and** `businessHours` derived from Stage 0 hours. Example shape: `{"storeOpen": true, "deliveryMode": null, "businessHours": {"weekday": {"open": "13:00", "close": "23:00"}, "friday": null, "saturday": {"open": "19:00", "close": "23:00"}}}` (set `friday` / `saturday` to `null` if user answered `סגור`). Write via: save JSON to a temp file, then `firebase database:set //settings <tempfile> --project <new_name> --force` (note: on MSYS-style bash on Windows a single leading `/` gets mangled into `https://`, use `//settings`). The admin's `runAutoSchedule()` reads `businessHours` every minute and fires open/close at the transition moments; manual button override always wins until the next transition. |
