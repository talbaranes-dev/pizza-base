...
## 4. סיסמאות
- ℹ️ **אתר ההזמנות ללא מסך כניסה** — מסך הכניסה הוסר מהתבנית. כל מי שפותח את הכתובת רואה ישר תפריט ומזמין. חוקי RTDB מאפשרים יצירת הזמנה ללא auth (רק עדכון הזמנה קיימת דורש auth).
- [🤖] `MANAGER_PASSWORD_HASH` + `MANAGER_PANEL_PASSWORD` — סיסמת פאנל מנהל בלבד
- [🤖] משתמש Firebase Auth (`admin_email` + `manager_password`) נוצר אוטומטית ב-Stage 5.5 (אתר המנהל בלבד)
...
> **שעות פעילות עובדות בשני מישורים:** (1) טקסט תצוגה בתחתית האתר, (2) לוח זמנים אוטומטי לפתיחה/סגירה של הבוט. Stage 5.6 זורע את השעות ל-`settings/businessHours` ב-Firebase, ופאנל המנהל מריץ `runAutoSchedule()` כל דקה — כשמגיעה שעת פתיחה, הבוט נפתח; כשמגיעה שעת סגירה, הבוט נסגר. **לחיצה ידנית על הכפתור תמיד גוברת** עד ה-transition הבא — המנהל יכול לפתוח/לסגור חריג בכל שעה. תנאי: פאנל המנהל חייב להיות פתוח ברקע בשעת ה-transition (או יותר מאוחר באותו יום — catch-up אוטומטי בטעינה).

> **תג סטטוס דינמי** מופיע מעל שעות הפעילות ומציג בזמן אמת (עדכון כל 30 שנ'): `🟢 פתוח עכשיו` כש-`storeOpen = true`, או `🔴 סגור כרגע` אחרת. הלקוח יודע מיידית אם אפשר להזמין, ללא תלות בטקסט השעות הסטטי.
